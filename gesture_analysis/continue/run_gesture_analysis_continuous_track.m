...
PARA.smooth_window_sec = 1;    % 滑动平均窗口(秒): 用于计算长时间背景基线
PARA.gvi_threshold     = 2;      % GVI 激活阈值: 超过此值才开始计算轨迹
PARA.sampling_rate     = 25;     % 数据采样率 (Hz)
PARA.merge_gap_sec     = 0.4;    % 动作合并窗口: 断开小于0.2秒视为同一动作

% 3. 轨迹重建参数 (用于 Step 2)
TRAJ.gesture_height    = 0.30;   % [物理] 手势平面的假设高度 (米)
TRAJ.min_elevation     = 0;     % [物理] 最低仰角门限 (度): 低于此角度的卫星直接丢弃
...
% 4. 时域聚类参数 (V2 核心)
TRAJ.time_cluster_k    = 5;     % [聚类] 时间窗口: 每 10 个原始采样点(约0.4秒)合并为一个轨迹点
...
TRAJ.elevation_power   = 2;      % [重要] 仰角权重指数。越高越只信头顶信号，抗手臂干扰越强。
