...
% 功能: 鲁棒手势感知 - 边界追踪最终版 (v3.1 - 逻辑修正版)
% 改进: 
%   1. [修复]: 修正了天顶角过滤逻辑 (原代码误删了头顶信号，现改为剔除低仰角干扰)。
%   2. [注释]: 全面更新了参数含义注释，增加了对"半径门控"的解释。
%   3. [架构]: 保持纯粹的流式掩膜 (Stream Mask) 处理，移除旧的分段检测。
% =========================================================================
...
fprintf('--> 启动前沿边界轨迹追踪 (v3.1: Logic Fixed)...\n');
...
PARA.noise_cutoff_db    = 1;     % 噪声容忍度 (dB): 信号波动小于此值时，被强制归零(视为平稳)
...
PARA.smooth_window_sec = 1.5;   % GVI基线窗口 (秒): 用于计算波动能量(GVI)的滑动平均窗口大小
PARA.gvi_threshold     = 2;     % GVI触发阈值 (dB): 全局波动能量超过此值，系统才进入"追踪模式"
PARA.sampling_rate     = 25;    % 数据采样率 (Hz): 必须与观测数据的实际采样率匹配
PARA.merge_gap_sec     = 0.2;   % 动作缝合阈值 (秒): 两个动作间隔小于0.2s时，视为同一个连续动作(防止画圆时中间断开)
PARA.min_duration_sec  = 0.4;   % 最小动作时长 (秒): 持续时间小于0.4s的波动被视为偶发干扰，直接丢弃

% [Step 2: 几何反演与轨迹追踪 (Geometry & Tracking)]
% 作用: 将一维的信号波动反演为二维的空间轨迹
TRAJ.gesture_height    = 0.20;  % 手势物理高度 (米): 假设手在接收机上方20cm处运动(用于视线投影)
TRAJ.min_elevation     = 15;    % 最低仰角门限 (度): 剔除地平线附近(仰角<15°)的多径严重卫星
TRAJ.min_action_dist   = 0.05;  % 动作触发死区 (米): 轨迹重心偏移超过5cm才开始记录，防止静止时的零漂

% [关键抗干扰参数]
ALG.zenith_safe_deg    = 50;    % 安全仰角门限 (度): 用于剔除容易受身体遮挡的低仰角区域 (保留仰角 > 30° 的卫星)
ALG.top_k_percent      = 0.2;   % 前沿追踪比例 (0.2=20%): 仅利用距离圆心最远的20%的卫星点来计算重心(模拟指尖位置，抗手臂干扰)

TRAJ.time_cluster_k    = 10;    % 时域聚类窗口 (点数): 每10个采样点合并计算一个轨迹坐标(降低轨迹抖动)
TRAJ.traj_smooth_m     = 5;     % 轨迹平滑窗口 (点数): 对生成的最终轨迹进行移动平均平滑
...
for t_start = 1 : K_cluster : num_samples
    ...
    for t = range_indices
        ...
        for k = 1:length(valid_energy_idx)
            ...
            % 旧逻辑: if zen_deg < 30 (错误! 删除了头顶最好的信号)
            % 新逻辑: 剔除仰角过低的卫星。如果仰角 < 30度，认为可能被身体挡住，跳过。
            ...
            % --- [半径门控] Radius Gating ---
            % 物理意义: 限制有效感知的水平半径。
            % 如果投影点距离接收机超过 5 米，说明这颗卫星仰角极低，或者信号路径极长，
            % 极有可能是远处的环境噪声反射，而非手部遮挡。
